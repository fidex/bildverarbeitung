...
typedef unsigned long xaULONG;
typedef signed short  xaSHORT;
typedef void          XA_ACTION;
//#include "xanim.h"
...
typedef struct
{
  char    fcc_type[5];    /* fccType  {vids} */
  char    fcc_handler[5]; /* fccHandler {msvc,RLE} */
  xaULONG flags;          /* Flags */
  xaSHORT priority;       /* Priority*/
  xaSHORT language;       /* language */
  xaULONG init_frames;    /* InitialFrames */
  xaULONG scale;          /* Scale */
  xaULONG rate;           /* Rate */ /* rate / scale == samples/second */
  xaULONG start;          /* Start */
  xaULONG length;         /* Length In units above (e.g images or audio samples) */
  xaULONG sug_bsize;      /* SuggestedBufferSize */
...
typedef struct /* BitMapInfoHeader */
{
  ...
  xaULONG width;          /* Width */
  xaULONG height;         /* Height */
  xaSHORT planes;         /* short Planes */
  xaSHORT bit_cnt;        /* short BitCount */
