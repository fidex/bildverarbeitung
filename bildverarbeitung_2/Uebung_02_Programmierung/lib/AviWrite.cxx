...
AviWrite& AviWrite::operator<<(Img<RGB_Pixel>& img) {
  if ((width!=img.Width())||(height!= img.Height())) {
    ErrorText = "int AviWrite::operator=(Img<RGB_Pixel>& img): Falsche Bildgröße übergeben";
    throw this;
  }
  long BufferSize = Align32(width*3)*height;
  ...
  for (y=0;y<height;y++) {
    for (x=0;x<width;x++) {
      LineBuffer[3*x]   = img[y][x].Blue();
      LineBuffer[3*x+1] = img[y][x].Green();
      LineBuffer[3*x+2] = img[y][x].Red();
    }
...
void AviWrite::WriteJunkHeader(const char* buffer, long size){
...
void AviWrite::WriteTag(const char* buffer){
  if (4 != write_buffer(buffer, 4)) {
    error = 1;
